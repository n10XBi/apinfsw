...
async function pollImageResultArting(request_id, env, {
    timeout = 180000,
    interval = 4000
} = {}) {
...
async function validateApiKey(env, request, url) {
    ...
    if (url.pathname.startsWith("/telegram/")) return {
        ok: true
    };
    ...
    if (!KV) {
        return {
            ok: false,
            res: new Response(JSON.stringify({
                error: "KV binding Gen-api-txt2img not found"
            }), {
                status: 500,
                headers: corsHeaders()
            })
            ...
            for (const img of images) {
                if (typeof img === "string" && img.startsWith("data:image")) {
                    ...
                    if (match) {
                        ...
                        await telegramApi(botToken, "sendMessage", {
                            chat_id: chatId,
                            text: "Gambar (base64) diterima tapi format nda dikenali."
                        });
                    }
                    ...
                    await telegramApi(botToken, "sendMessage", {
                        chat_id: chatId,
                        text: "Hasil gambar nda dikenali."
                    });
                }
            }
...
async function handleTelegramCallback(botToken, cq) {
    ...
    switch (cmd) {
        ...
        case "show_lora":
            await telegramApi(botToken, "sendMessage", {
                chat_id: chatId,
                text: "Pilih sampai 3 LORA ya. Klik lagi untuk hapus.",
        ...
        case "lora_toggle": {
            ...
            if (session.lora && session.lora[id] !== undefined) {
                ...
                if (Object.keys(session.lora).length >= 3) {
                    await telegramApi(botToken, "sendMessage", {
                        chat_id: chatId,
                        text: "Maksimum 3 LORA ya cintakkuuu 😅."
                    });
                    break;
                }
                session.lora[id] = 0.7;
        ...
        case "samples_set": {
            ...
            if (!isNaN(samples)) {
                ...
                await telegramApi(botToken, "sendMessage", {
                    chat_id: chatId,
                    text: "Pilihan nda valid, coba lagi ya.",
                    ...buildSettingsKeyboard(session)
                });
            }
            break;
        ...
        case "set_negative_prompt":
            ...
            await telegramApi(botToken, "sendMessage", {
                chat_id: chatId,
                text: "Kirim Negative Prompt kamu ya (contoh: blur, bad anatomy, deformed)"
            });
            break;
