...
async function getImageResultArting(request_id, env) {
  ...
  const data = await res.json();
  return {
    status: data.code === 100000 && data.data?.output?.length > 0 ? "done" : "pending",
    request_id,
...
if (pathname === "/get_result") {
  ...
  try {
    // 🔥 langsung hardcode token di sini
    ...
    const resp = await fetch("https://api.arting.ai/api/cg/text-to-image/get", {
      method: "POST",
      headers: {
        "Authorization": token,
        "Content-Type": "application/json",
